...
namespace Microsoft.ApplicationInsights.Profiler.Core.Orchestration
{
    ...
    internal class OrchestratorEventPipe : Orchestrator
    {
        ...
        public async override Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            if (activatedCount > 0)
            {
                _logger.LogDebug("{0} scheduling policies has been activated.", activatedCount);
                ...
                List<Task> policyAsync = new List<Task>();
        ...
        {
            _logger.LogTrace("Start profiling is requested by {0}", policy.Source);
                ...
                if (await waitSemaphoreTask.ConfigureAwait(false))
                {
                    try
                    {
                        if (_currentProfilingPolicy == null)
                        {
                            ...
                            if (result)
                            {
                                _logger.LogDebug("Profiling is started. Source: {0}", policy.Source);
                        ...
                        {
                            _logger.LogTrace("Profiling is running by policy: {0}", _currentProfilingPolicy.Source);
                        }
                    }
                    finally
                    {
        ...
        public override async Task<bool> StopProfilingAsync(SchedulingPolicy policy, CancellationToken cancellationToken)
        {
            ...
            if (_currentProfilingPolicy == null)
            {
                _logger.LogTrace("Request stop profiling by {0} rejected. No profiling is running.", policy.Source);
                result = false;
            }
            else if (_currentProfilingPolicy != policy)
            {
                _logger.LogTrace("Stop profiling by {0} failed. The current profiling is requested by {1}", policy.Source, _currentProfilingPolicy?.Source);
                result = false;
            }
            else
            {
                ...
                try
                {
                    ...
                    if (await waitSemaphoreTask.ConfigureAwait(false))
                    {
                        try
                        {
                            ...
                            {
                                _logger.LogDebug("Stopping Profiler by {0}", policy.Source);
                                result = await _profilerProvider.StopServiceProfilerAsync(policy, cancellationToken).ConfigureAwait(false);
                                if (result)
                                {
                                    _logger.LogDebug("Profiler stopped by {0}", policy.Source);
                                    _currentProfilingPolicy = null;
                                }
                            }
                            else
                            {
                                _logger.LogTrace("Can't stop profiling requested by {0}.", _currentProfilingPolicy.Source);
                                result = false;
                            }
                        }
                        finally
                    ...
                    {
                        _logger.LogWarning("Can't get the handler to stop a profiling. Requested by {0}. Current Profiling by: {1}", policy.Source, _currentProfilingPolicy?.Source);
                        result = false;
                    }
                }
                finally
        ...
        private SemaphoreSlim _policyChangeHandler = new SemaphoreSlim(1, 1);
